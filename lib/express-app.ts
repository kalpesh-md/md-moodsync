...
async function fetchIncomingFriendRequests(db, userId) {
...
async function fetchOutgoingPendingRequests(db, userId) {
  const { data: follows, error } = await db
    .from("mood_follows")
    .select("following_id")
...
async function fetchMutualFriends(db, userId) {
...
app.get("/api/friends/summary", authRequired, async (req, res) => {
  try {
    const db = getSupabaseAdmin();
    const userId = req.user.userId;
    ...
    console.error("friends/summary error:", err.message);
    res.json({ friends: [], requests: [], pending: [] });
  }
});
